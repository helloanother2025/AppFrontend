...
export default function EditRide() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (isMounted && fetched) {
        setRide(fetched);
        setFare(fetched.fare !== undefined && fetched.fare !== null && fetched.fare !== 'TBA'
          ? String(fetched.fare) : '');
        setTransport(fetched.transportMode || fetched.transport || '');
        setRideProvider(fetched.rideProvider || '');
        setPreferences({
          numPartners: fetched.totalPassengers || fetched.availableSeats || 1,
          gender: fetched.gender || fetched.genderPreference || 'Any',
          otherNotes: fetched.preferences || fetched.notes || '',
        });
      }
    })();
  ...
  // Reset seat count when transport type changes (max differs per mode)
  useEffect(() => {
    setPreferences(p => ({ ...p, numPartners: Math.min(p.numPartners, maxPartners) }));
  }, [transport]);

  const maxPartners = transport === 'Car' || carOptions.includes(transport) ? 4
    : transport === 'CNG' ? 3
    : 10;
    ...
    <ScrollView style={{ paddingHorizontal: 20 }}>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.inputContainer}>
          ...
          <View style={{ flexDirection: 'row', marginVertical: 5 }}>
            {['Any', 'Male', 'Female'].map(opt => (
              ...
              >
                <Text style={[styles.pillText, preferences.gender === opt && styles.pillTextActive]}>
                  {opt}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        <View style={styles.inputContainer}>
          ...
          >
            <View style={styles.transportRow}>
              ...
              <Text style={styles.transportText}>
                {carOptions.includes(transport) ? transport : 'Car'}
              </Text>
              <Ionicons
                name={showCarOptions ? 'chevron-up' : 'chevron-down'}
                size={18}
                color="black"
          ...
          <Button
            onPress={() => setTransport('CNG')}
            style={transport === 'CNG' ? styles.selectedCard : {}}
          >
            <View style={styles.transportRow}>
              <Text style={styles.transportIcon}>🛺</Text>
          ...
          <Button
            onPress={() => setTransport('Bus')}
            style={transport === 'Bus' ? styles.selectedCard : {}}
          >
            <View style={styles.transportRow}>
              <Text style={styles.transportIcon}>🚌</Text>
        ...
        <View style={styles.inputContainer}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 10 }}>
            <TextInput
              ...
              placeholder={ride.fare && ride.fare !== 'TBA' ? ride.fare.toString() : 'Enter fare...'}
        ...
        {/* Current passengers (display only) */}
        {ride.partners && ride.partners.length > 0 && (
          <View style={styles.inputContainer}>
            <Text style={styles.formText}>Current passengers</Text>
                ...
                >
                  ...
                  <View>
                    ...
                    <Text style={styles.handle}>
                      @{partner.username || (partner.handle || '').replace('@', '')}
                    </Text>
                  </View>
                </TouchableOpacity>
              </View>
